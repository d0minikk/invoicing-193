module Invoicing
  ...
  module ClassInfo
    ...
    def self.acts_as(source_module, calling_class, args)
      ...
      previous_info = if calling_class.private_instance_methods(true).include?(class_info_method.to_sym)
      ...
      calling_class.class_eval <<-CLASSEVAL
        class << self
          def #{class_info_method}
            if superclass.private_instance_methods(true).include?("#{class_info_method}".to_sym)
